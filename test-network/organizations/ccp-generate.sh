...
function json_network {
    sed -e "s/\${ORG}/$1/" \
        -e "s#\${ORG}#$1#" \
        organizations/network-template.json
}
...
echo "$(json_network $ORG)" > organizations/peerOrganizations/org1.example.com/network.json
...
# echo "$(json_network $ORG)" > organizations/peerOrganizations/org2.example.com/network.json
